...
class Spinner{
  ...
  **/
  public function spin(tasks:Array<Task>):Reactor<Bool>{
    return Reactor.lift(
      (cb) -> {
        var done  = false;
        var waits : Array<Reactor<Noise>> = [];
        var polls : Array<Int>            = [];
        ...
        if(!done){
          switch([waits.is_defined(),polls.is_defined()]){
            case [true,true] : 
              ...
              wait_any.or(poll_any).map(_ -> true)(cb);
            case [true,false] : 
              get_wait_any(waits).map(_ -> true)(cb);
            case [false,true]   : 
              get_poll_any(polls).map(_ -> false)(cb);
            case [false,false]  :
              var backoff = __.run().delay(backoff.delta);
              var limits  = __.run().delay(limit.hanging);
              backoff.or(limits).map(_ -> false)(cb);
          }
        }
      }
    );
  ...
  function get_wait_any(waits:Array<Reactor<Noise>>){
    return Reactor.inj.any(waits).def(Reactor.unit);
  }
